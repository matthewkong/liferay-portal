...
<aui:fieldset column="<%= true %>" cssClass="aui-w50">
	...
	<c:if test="<%= !PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || (selUser != null) %>">
		<c:choose>
			<c:when test='<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.USERS_SCREEN_NAME_ALWAYS_AUTOGENERATE) || ((selUser != null) && !UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "screenName")) %>'>
	...
	<c:choose>
		<c:when test='<%= ((selUser != null) && (!UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "emailAddress"))) %>'>
...
<aui:fieldset column="<%= true %>" cssClass="aui-w50">
	<div>
		<c:if test="<%= selUser != null %>">
			<c:choose>
				<c:when test='<%= (UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "portrait")) %>'>
	...
	<c:choose>
		<c:when test="<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.FIELD_ENABLE_COM_LIFERAY_PORTAL_MODEL_CONTACT_BIRTHDAY) %>">
			...
			<aui:input bean="<%= selContact %>" disabled='<%= (!UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "birthday")) %>' model="<%= Contact.class %>" name="birthday" value="<%= birthday %>" />
		</c:when>
		<c:otherwise>
	...
	<c:if test="<%= PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.FIELD_ENABLE_COM_LIFERAY_PORTAL_MODEL_CONTACT_MALE) %>">
		<aui:select bean="<%= selContact %>" disabled='<%= (!UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "gender")) %>' label="gender" model="<%= Contact.class %>" name="male">
			<aui:option label="male" value="true" />
			<aui:option label="female" value="false" />
		</aui:select>
	</c:if>

	<aui:input disabled='<%= (!UsersAdminUtil.hasUpdatePermission(permissionChecker, user, "jobTitle")) %>' name="jobTitle" />
</aui:fieldset>